    ...
    public class Order : IComparable
    {
        ...
        public void AddDetail(OrderDetail orderDetail)
        {
            if (OrderDetails.Contains(orderDetail))
                throw new ArgumentException($"订单项{orderDetail.OrderGoods.Name} 已经存在!");
            OrderDetails.Add(orderDetail);
        }
        public override string ToString()
        {
            StringBuilder description = new StringBuilder( $"OrderId: {OrderID}\tOrderClient: {OrderClient}\tOrderTime: {Time}\nOrderDetails: \n");
            //列出每条订单明细的信息
